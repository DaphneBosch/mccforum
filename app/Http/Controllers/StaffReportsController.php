...
class StaffReportsController extends Controller
{
    ...
    {
        $poster = DB::table('posts')->where('category', 'staffreports')->orderByDesc('id')->paginate(10);
        $posterrecent = Post::orderBy('id', 'desc')->take(5)->get();
        $registeredusers = DB::table('users')->count();

        return view('front.forum.staffreports.staffreports')->with('poster', $poster)->with('posterrecent', $posterrecent)->with(
